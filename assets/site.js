...
(function(){
  ...
  d.querySelectorAll('[data-sms]').forEach(a=>a.href=sms(a.dataset.sms||"Hi — I'd like to book. Dates: ___ · Group size: ___ · Looking for: villa / cars / yacht / tables"));
  ...
  if(finder&&fOut&&fGo){ const state={city:'Miami',what:new Set(),group:'',dates:''}; const out=fOut, go=fGo;
    const render=()=>{ const what=[...state.what].join(', ')||'___'; const msg=`Hi — building a weekend with Juice Luxury Rentals.\nCity: ${state.city}\nDates: ${state.dates||'___'}\nGroup: ${state.group||'___'}\nNeed: ${what}`; out.textContent=msg; go.href=sms(msg); };
